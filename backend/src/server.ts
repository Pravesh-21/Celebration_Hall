...
if (process.env.NODE_ENV === 'production') {
  ...
  const insecure = [process.env.JWT_SECRET, process.env.ADMIN_API_KEY, process.env.ADMIN_PASSWORD].filter(
    (v) => INSECURE_DEFAULTS.includes(v ?? '')
  );
...
function adminAuth(req: Request, res: Response, next: NextFunction): void {
  ...
  try {
    ...
    if (payload.role !== 'admin') throw new Error('Insufficient role');
...
const bookingSchema = z.object({
  ...
  guestCount: z.number().int().min(10, 'Minimum guest count is 10').max(2000, 'Maximum guest count is 2000'),
  dateFlexibility: z.enum(['fixed', 'flexible']),
  venueId: z.string().min(1, 'Venue selection is required').max(60),
...
app.post('/api/admin/login', loginLimiter, (req: Request, res: Response) => {
  ...
  if (usernameMatch && passwordMatch) {
    const token = jwt.sign(
      { username, role: 'admin' },
      JWT_SECRET,
      { expiresIn: JWT_EXPIRES_IN, issuer: 'grandeur-hall-api' }
    );
...
app.post('/api/bookings', bookingLimiter, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (error.code === 11000) {
      ...
      const booking = await Booking.create({ ...result.data, referenceNumber: fallbackRef, status: 'pending' });
...
app.get('/api/admin/bookings', adminAuth, async (req: Request, res: Response) => {
  try {
    const {
      status,
      eventType,
    ...
    const safeSortBy = ALLOWED_SORT_FIELDS.includes(sortBy as string) ? (sortBy as string) : 'createdAt';
    ...
    if (status && status !== 'all' && ['pending', 'confirmed', 'rejected'].includes(status as string)) {
    ...
    const [bookings, totalCount] = await Promise.all([
      Booking.find(filter).sort({ [safeSortBy]: sortDir }).skip(skip).limit(limitNum).lean(),
...
app.get('/api/admin/bookings/:id', adminAuth, async (req: Request, res: Response) => {
  try {
...
app.patch('/api/admin/bookings/:id/status', adminAuth, async (req: Request, res: Response) => {
...
app.delete('/api/admin/bookings/:id', adminAuth, async (req: Request, res: Response) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
      return res.status(400).json({ success: false, message: 'Invalid booking ID.' });
    }
